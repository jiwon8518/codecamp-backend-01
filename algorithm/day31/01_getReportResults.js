...
function solution(id_list, report, k) {
  const report = {}; // 신고한 사람이 누구를 신고했는지에 대한 정보를 저장
  const users = {}; // 신고 당한 사람이 몇번 신고를 당했는지에 대한 정보를 저장
  const answer = [];
  ...
  for (let i = 0; i < report.length; i++) {
    const info = report.split(' ');
    // console.log(info);
    if (report[info[0]] === undefined) {
      report[info[0]] = [];
    }
    if (users[info[1]] === undefined) {
      users[info[1]] = 0;
    }
    // // new Set 사용시 중복제거 사용 안해도됨
    // // 중복 신고를 제거
    // // 지금 신고하려는 사람이 이전에 신고한 경력이 있는지 없는지를 판단
    // if (report[info[0]].includes(info[1]) === false) {
    report[info[0]].push(info[1]);
    users[info[1]]++;
    // }
  }
  for (let i = 0; i < id_list.length; i++) {
    const arr = report[id_list[i]] || 0; // undefined 일 경우 0으로 받겠다
    answer[i] = 0;

    for (let l = 0; l < arr.length; l++) {
      // console.log(users[arr[l]], id_list[i]);
      if (users[arrl] >= k) {
        answer[i]++;
      }
    }
...
function solution(id_list, report, k) {
  ...
  report.forEach((el) => {
    el = el.split(' '); // 공백은 기준으로 나눠줌
    console.log(el);

    if (report[el[0] === undefined]) {
      report[(el[0] = [])];
    }
    ...
    report[el[0].push(el[1])];
    users[el[1]]++;
  });

  const answer = id_list.map((name) => {
    const arr = report[name];
    console.log(name, arr);
    return arr.reduce((acc, cur) => {
      const arr = report[name] || []; // undefined 이라면 빈 배열을 넣어줌
